...
CMC_Obs <- Combined_Data_Success_Dummies %>% filter(CMC_Dummy == "1") %>% nrow()
CMC_econ <- CMC_Obs %>% group_by(economics_dummy) %>% summarize(n=n()) %>% mutate(prop=n/sum(n))

Pomona_Obs <- Combined_Data_Success_Dummies %>% filter(Pomona_Dummy == "1") %>% nrow()

Pitzer_Obs <- Combined_Data_Success_Dummies %>% filter(Pitzer_Dummy == "1") %>% nrow()

Scripps_Obs <- Combined_Data_Success_Dummies %>% filter(Scripps_Dummy == "1") %>% nrow()

HMC_Obs <- Combined_Data_Success_Dummies %>% filter(HMC_Dummy == "1") %>% nrow()
...
School_Count <- (Combined_Data_Success_Dummies)[c(CMC_Dummy, Pomona_Dummy, Scripps_Dummy, HMC_Dummy, Pitzer_Dummy)] 
